package scau.edu.cn.notebook.dao;
...
import scau.edu.cn.notebook.helper.JeekDBConfig;
import scau.edu.cn.notebook.helper.JeekSQLiteHelper;
...
public class ScheduleDao {
    ...
    public int addSchedule(Schedule schedule) {
        ...
        //edited by cjb, 加入用户的手机号码
        values.put(JeekDBConfig.SCHEDULE_USERNUMBER, schedule.getUser_number());
    ...
    public List<Schedule> getScheduleByDate(String usernumber, int year, int month, int day) {
        List<Schedule> schedules = new ArrayList<>();
        SQLiteDatabase db = mHelper.getReadableDatabase();
        Cursor cursor = db.query(JeekDBConfig.SCHEDULE_TABLE_NAME, null,
                String.format("%s=? and %s=? and %s=? and %s=?", JeekDBConfig.SCHEDULE_USERNUMBER,JeekDBConfig.SCHEDULE_YEAR,
                        JeekDBConfig.SCHEDULE_MONTH, JeekDBConfig.SCHEDULE_DAY), new String[]{usernumber, String.valueOf(year),
                        String.valueOf(month), String.valueOf(day)}, null, null, null);
        Schedule schedule;
        while (cursor.moveToNext()) {
            schedule = new Schedule();
            schedule.setUser_number(cursor.getString(cursor.getColumnIndex(JeekDBConfig.SCHEDULE_USERNUMBER)));
